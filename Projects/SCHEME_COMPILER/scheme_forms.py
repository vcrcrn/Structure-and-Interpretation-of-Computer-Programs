...
def do_and_form(expressions, env):
    ...
    False
    """
    # BEGIN PROBLEM 12 --> python3 ok -q 12
    "*** YOUR CODE HERE ***"
    if expressions == nil:
        return True
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            # BEGIN PROBLEM 13 --> python3 ok -q 13
            "*** YOUR CODE HERE ***"
            if clause.rest == nil:
                return test
            else:
...
def make_let_frame(bindings, env):
    ...
    names = vals = nil
    # BEGIN PROBLEM 14 --> python3 ok -q 14
    "*** YOUR CODE HERE ***"
    while bindings:

        binder = bindings.first
        validate_form(binder, 2, 2)
        names = Pair(binder.first, names)
        vals = Pair(scheme_eval(binder.rest.first, env), vals)
