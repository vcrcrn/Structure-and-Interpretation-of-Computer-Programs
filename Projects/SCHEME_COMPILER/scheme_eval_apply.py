...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        operador = scheme_eval(first, env)
        validate_procedure(operador)
        operands = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(operador, operands, env)
        # END PROBLEM 3
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        python_list_with_args = []
        while args is not nil:
            python_list_with_args.append(args.first)
            args = args.rest
        
        if procedure.expect_env:
            python_list_with_args.append(env)

        try:
            return procedure.py_func(*python_list_with_args)
