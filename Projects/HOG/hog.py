...
def hefty_hogs(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    index = opponent_score % 10
    func = digit_fn(index)
    value = func(player_score) 
    opponent_score //= 10
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    elif num_rolls and opponent_score == 0:
        return 1
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        if who == 0:
            ...
            if score1 == 0 and rolls0 == 0:
                score0 += 1
            else:
                score0 += take_turn(rolls0, score0, score1, dice, goal=GOAL_SCORE)
                score0 += hog_pile(score0, score1)
            who = next_player(0)
        
        else:
            rolls1 = strategy1(score1, score0)
            if score0 == 0 and rolls1 == 0:
                score1 += 1   
            else:
                score1 += take_turn(rolls1, score1, score0, dice, goal=GOAL_SCORE)
    ...
    #play(always_roll(1), always_roll(1), dice=make_test_dice(5, 6, 5), goal=10, say=announce_lead_changes)

    while score0 < goal and score1 < goal:

        say(score0, score1, who)
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    while new_leader != last_leader:

        message = f"Player {new_leader} takes the lead by {abs(score0 - score1)}"

        if score0 > score1:
            new_leader = 0
            return new_leader, message
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while iterator <= 10:
        ...
        avg = averaged_roll_dice(iterator, dice)
        if avg > limit:
            limit = avg
            number_limit = iterator
        iterator += 1
    return number_limit
    # END PROBLEM 9
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    if threshold <= hefty_hogs(score, opponent_score):
        return 0
