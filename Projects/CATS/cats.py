...
def about(topic):
    ...
    def select(paragraph):
        ...
        elements = remove_punctuation(paragraph).lower().split()
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    filtered = {element: diff_function(typed_word, element, limit) for element in word_list}
    
    if typed_word in word_list:
        return typed_word
    
    elif limit < filtered[min(filtered, key= lambda element: filtered[element])]:
        return typed_word
    
    else:
        return min(filtered, key= lambda element: filtered[element])
    # END PROBLEM 5
...
def sphinx_swaps(start, goal, limit):
    ...
    def counter(start, goal, count, index):
        ...
        if start[index] != goal[index]:
            return counter(start, goal, count+1, index+1)

        else:
            return counter(start, goal, count, index+1)

    return counter(start, goal, count=0, index=0)
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    def helper(sofar, prompt, count, i):
    ...
    return helper(sofar, prompt, count=0, i=0)
...
def time_per_word(words, times_per_player):
    ...
    for player in times_per_player:
        ...
        time_diffs.append([player[time] - player[time - 1] for time in range(1, len(player))])

    return match(words, time_diffs)
    ...
    for word in word_indices:
        ...
        for player in player_indices:
            if time(match, player, word) < min_time:
                min_time, curr_player = time(match, player, word), player
        
        speedy_fingers[curr_player].append(word_at(match, word))
        
    return speedy_fingers
